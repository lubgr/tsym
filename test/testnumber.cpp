...
TEST(IntOverflow, intCreationWithOverflowedInt)
{
    Int arg(Int::max());
    Number res;
...
TEST(IntOverflow, fractionCreationWithOverflowedInts)
{
    Int num(Int::max());
    Int denom(Int::max());
    Number res;
...
TEST(IntOverflow, simplePowerOverflow)
{
    const double expected = std::pow(2.0, 400.0);
    Number pow(2);

    pow = pow.toThe(400);

    CHECK(pow.isDouble());
    DOUBLES_EQUAL(expected, pow.toDouble(), TOL);
}

TEST(IntOverflow, fracBaseGreaterThanOne)
    /* This case should be handled by switching to an inexact evaluation of the power, i.e.
     * transform the fraction to double first, then evaluate the power which results in a moderatore
     * value. */
{
    const Number exp(7, 5);
    Number res(169, 25);

    res = res.toThe(exp);

    CHECK(res.isDouble());
    DOUBLES_EQUAL(14.518621198064931, res.toDouble(), TOL);
}

TEST(IntOverflow, fracBaseLessThanOne)
    /* (21/5153632)^(3/2) = 8.2254e-09. */
{
    const Number exp(3, 2);
    Number res(21, 5153632);

    res = res.toThe(exp);

    CHECK(res.isDouble());
    DOUBLES_EQUAL(8.225434264261553e-09, res.toDouble(), TOL);
}

TEST(IntOverflow, posIntMultiplication)
{
    const double expected = maxInt.toDouble()*2.0;
    Number res(maxInt);

    res *= 2;

    CHECK(res.isDouble());
    DOUBLES_EQUAL(expected, res.toDouble(), TOL);
}

TEST(IntOverflow, negIntMultiplication)
{
    const double expected = minInt.toDouble()*2.0;
    Number res(minInt);
...
TEST(IntOverflow, fracMultiplication)
{
    const double expected = 2.0*2039423/(5.0*(maxInt - 4).toDouble());
    Number res(2039423, maxInt - 4);
...
TEST(IntOverflow, posIntAddition)
{
    const double expected = maxInt.toDouble() - 1000.0 + 1234.0;
    Number res(maxInt - 1000);
...
TEST(IntOverflow, negIntAddition)
{
    const double expected = minInt.toDouble() - 12.0;
    Number res(minInt);
...
TEST(IntOverflow, simpleFracAddition)
{
    const double expected = 1039111.0/(maxInt - 1).toDouble() + 27.0/(maxInt - 2).toDouble();
    const Number s1(1039111, maxInt - 1);
    const Number s2(27, maxInt - 2);
    Number res;
...
TEST(IntOverflow, fracSumFirstSummandOverflow)
{
    const int smallestCommonMultiple = 2*3*5*7*11;
    const double expected = (maxInt - 5).toDouble()/(13.0*17.0*21.0*(double)smallestCommonMultiple)
        + 1.0/(double)smallestCommonMultiple;
    const Number s1(maxInt - 5, 13*17*21*smallestCommonMultiple);
    const Number s2(1, smallestCommonMultiple);
    Number res;
...
TEST(IntOverflow, fracSumSecondSummandOverflow)
{
    const int smallestCommonMultiple = 3*5*7*17;
    const double expected = (maxInt - 10).toDouble()/(11.0*13.0*23.0*43.0*
            smallestCommonMultiple) + 1.0/smallestCommonMultiple;
    const Number s1(1, smallestCommonMultiple);
    const Number s2(maxInt - 10, 11*13*23*43*smallestCommonMultiple);
    Number res;
...
TEST(IntOverflow, fracAdditionNoOverflow)
{
    const Number s1(1, maxInt);
    const Number s2(1, maxInt);
    Number res;
    ...
    CHECK_EQUAL(maxInt, res.denominator());
}

TEST(IntOverflow, tryConvertDoubleToFrac)
{
    const Int num = maxInt - 1;
    const int denom = 5;
    const double expected = num.toDouble()/denom;
    const Number res(expected);
