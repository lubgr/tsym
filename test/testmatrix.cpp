...
TEST(Matrix, multiplyByScalar)
{
    ...
    CHECK_EQUAL(Number(1, 2)*b2, m(0, 1));
...
TEST(Matrix, multipyTwoMatrices)
{
    ...
    CHECK_EQUAL(a*a + b*(b + c) + Number(1, 2)*b, res(0, 1));
...
TEST(Matrix, linearEqSetDim3)
{
    ...
    rhs(1) = Number(3, 5)*b*(d + c) + pow(a, 2*d)*f*f;
...
TEST(Matrix, largeNumericDet)
{
    ...
    const Var APtr[size][size] = {
        { 0, -3, 0, 0, Number(2, 3), 3, 0, 2, 0.163763761215668, 1, 0.972645326126339, Number(-1,
                9), -2, Number(-2, 9), 0.518333759061494, -3, 2, 2, 0.784473042052998, 2 },
        { -0.817845830240927, 0, 0, 0.871111379270801, -0.587976833818062, 0, -0.730432211807773,
            -0.436438037403665, 0, 0, Number(-1, 3), 0, 0, 0.336413789908801, 0, 1, 0, 0, -1,
            Number(-1, 3) },
        ...
        { -0.701953171249646, Number(-2, 9), 0, 0.233319224614176, 0.421511139950083,
            -0.236214381078049, 2, 0.935533452453758, -1, -1, 3, -3, Number(1, 3),
            0.431540855728871, -1, 0, -0.304962232988921, Number(2, 9), 2, Number(-1, 3) },
        { 0.409536719769605, 0.169569519475026, Number(1, 3), 0, 0, 0.487494740627881, Number(1, 3),
            0, 1, -3, 0, 1, 0, -3, 0.197019152177339, Number(2, 9), 0, -0.217695943889126,
            -0.323049213293008, -2 },
        { -3, 2, 0.564340432881273, -0.726041746735617, 0.661481848233678, 0.251710811672905,
            ...
            -0.517223090263141, Number(-1, 6), Number(-1, 3), -0.564726256451931, -1, 3 },
        { -0.362061416633498, 2, 0.878408900602927, Number(-1, 9), 0.285686875667206, -2, -2, 0,
            Number(-1, 6), 2, -1, 3, 0.464463147669131, 0, Number(1, 9), Number(1, 3), 0, 0, -3, -1
        },
        { -0.879212124759158, -0.814199905230186, 2, Number(2, 9), -0.198250188424975,
            0.501922631262054, 0, -2, Number(2, 9), -0.643714327219942, Number(-2, 3), -3, 2,
            Number(1, 9), Number(1, 6), -1, 0, Number(-1, 6), -0.891693022830127, -0.795647000362407
        },
        { 0.400446050091482, -2, Number(-1, 3), -0.179156659692230, -1, -0.102493130926656, 0, 1, 0,
            -2, -0.602360060388995, 0, 0, -1, -3, 0.472224469456821, -0.311738610520558, Number(1,
                    9), Number(-2, 9), -2 },
        { -2, -0.619736720716315, 0, -0.514268868053285, 1, -0.576144179562380, 2, 2, Number(1, 9),
            0.851523046057734, -2, Number(-1, 3), Number(1, 3), -0.661189346178467, -1, 2, -2, -3,
            0, 0.141647243131056 },
        { -3, -2, 0.756071627569365, 2, -3, 0, -0.286235664684729, 0.330354602667583,
            -0.404611840570539, -0.178961233748883, 0, 0, -3, 0, Number(-1, 3), -1, -3, 1, 2,
            -0.206800764892073 },
        { -0.780178283220871, 2, -1, 0, 0, -0.614305973735434, 0.334233970643127, 0.351167397856154,
            0.896517487896082, Number(1, 3), Number(-2, 9), 0, 0.556766076465591, 0,
            -0.255970680754231, 0.493476754055555, -2, -1, 0, -0.266899323261365 },
        { Number(-1, 3), 0, Number(-1, 3), -0.249453521690999, -3, 0.480133238056094,
            -0.712493294431367, 0.684729493715378, Number(1, 9), 0.634799018780988, Number(-1, 3),
            0, 0, Number(-2, 3), 1, 0.759958926984356, 1, -0.177062062068694, -0.738120757753549, -3
        },
        { Number(-1, 9), Number(2, 9), 0, Number(-1, 3), -2, Number(-1, 6), 0, 0,
            -0.991033486014383, Number(-1, 3), 2, 0, 0, 0, 0, -0.523520922745934, 1,
            0.841536360677068, -0.570936096606758, 0 },
        { -0.914577623243884, 0.643215642845554, Number(2, 3), Number(-2, 3), -1, Number(-1, 2),
        ...
        { 0.560116920154630, 0.182250758566945, 0.790114920815374, 0, 0, 0, -1, 2,
            -0.751359753172712, -0.350507602358450, 0.445573509241984, Number(-1, 3),
        ...
        { -1, -0.918781660997322, -0.469630186237979, -0.501190892717313, -0.506809526188111, 0,
            -0.745707881530680, 0, 3, Number(-1, 2), 0.118891897894698, -0.671535749003615,
            0.420302665526368, 0.579728657232557, -0.971242483607050, 0.957223256031235, 0,
            Number(-2, 3), -0.726977244558762, 0.729950389364666 },
        { Number(1, 6), 0, 0, -1, 0.182417591159116, 0, Number(-2, 9), Number(-1, 6), 0, Number(2,
                3), 0, -3, -2, -2, 0.695995893453037, 3, -1, 0, -0.302923538999540,
        -0.193937987755568 },
        { 0, -0.877908243205685, 0, Number(-1, 3), 0, -1, -3, 0.305292463545492, 0.298548866432640,
            -3, 1, 0, Number(-1, 6), -0.390714584652508, -0.964800524467479, Number(-1, 6), 2,
            -0.891977924554555, 0, 0.318824163394123 },
        { Number(2, 9), Number(2, 3), 0.593093099557646, 0, 0.612785416052094, 0, Number(2, 3), -1,
            0, 0.854807696107082, 0.631006623400826, 0, 2, 0, -1, -0.290114424749472, Number(1, 2),
            Number(-1, 9), -0.420632521291579, 0 }
    };
    Matrix A(size, size);
