        ...
        template <class Iterator> class QiParser {
          ...
          public:
            QiParser()
            {
                ...
                functions.add("sin", Fct::SIN)("cos", Fct::COS)("tan", Fct::TAN)("asin", Fct::ASIN)("acos", Fct::ACOS)(
                  "atan", Fct::ATAN)("sqrt", Fct::SQRT)("log", Fct::LOG);

                scalar = no_case[qi::lit("pi")][bind(&QiParser::pushPi)]
                  | no_case[qi::lit("euler")][bind(&QiParser::pushEuler)] | symbol[bindWithArg(&QiParser::pushSymbol)]
                  | floatingPoint[bindWithArg(&QiParser::pushDouble)] | integer[bindWithArg(&QiParser::pushInteger)];

                oneArgFunction = (no_case[functions] >> '(' >> expression >> ')')[bindWithArg(&QiParser::toFunction)];

                twoArgFunction = (no_case[qi::lit("atan2")] >> '(' >> expression >> ',' >> expression
                  >> ')')[bind(&QiParser::toAtan2)];

                function = oneArgFunction | twoArgFunction;

                operand = function | scalar | '(' >> expression >> ')';

                factor = operand >> *('^' >> factor[bind(&QiParser::toPower)]);

                posNegOperand = factor | '-' >> factor[bind(&QiParser::flipSign)];

                summand = posNegOperand >> *(('*' >> posNegOperand)[bind(&QiParser::toProduct)]
                                             | ('/' >> posNegOperand)[bind(&QiParser::toQuotient)]);

                expression = -qi::lit('+') >> summand
