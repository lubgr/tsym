...
namespace tsym {
    namespace plu {
        namespace detail {
            ...
            template <class Container> class DefaultAccess {
              ...
              private:
                template <class Matrix, typename SizeType>
                auto access(Matrix& m, SizeType i, SizeType j, int) -> decltype(m(i, j))
                {
                    return m(i, j);
                }

                template <class Matrix, typename SizeType>
                auto access(Matrix& m, SizeType i, SizeType j, long) -> decltype(m[i][j])
                {
                    return m[i][j];
                }
                ...
                template <class Vector, typename SizeType>
                auto access(Vector& vec, SizeType i, long) -> decltype(vec[i])
                {
                    return vec[i];
                }
            };
            ...
            template <class Vector, typename SizeType> class VectorAccessProxy {
              public:
                ...
                VectorAccessProxy(Vector& vec, Callable&& accessFct)
                    : vec(vec)
            ...
            template <class Matrix, typename SizeType> class MatrixAccessProxy {
              public:
                ...
                {}
                template <class Callable>
                MatrixAccessProxy(Matrix& matrix, Callable&& accessFct)
                    : m(matrix)
                    , access(std::forward<Callable>(accessFct))
                {}
            ...
            class VectorWrapper : public VectorAccessProxy<std::unique_ptr<Var[]>, SizeType> {
              public:
                explicit VectorWrapper(SizeType dim)
                    : VectorAccessProxy<std::unique_ptr<Var[]>, SizeType>(
                        rep, [](decltype(rep)& m, SizeType i) -> Var& { return m.get()[i]; })
            ...
            template <typename SizeType>
            class SquareMatrixWrapper : public MatrixAccessProxy<std::unique_ptr<Var[]>, SizeType> {
              public:
                explicit SquareMatrixWrapper(SizeType dim)
                    : MatrixAccessProxy<decltype(rep), SizeType>(
                        rep, [dim](decltype(rep)& m, SizeType i, SizeType j) -> Var& { return m.get()[i * dim + j]; })
                ...
                VectorAccessProxy<std::unique_ptr<Var[]>, SizeType> columnProxy(SizeType col)
                {
                    ...
                    auto accessFct = [col, localDim](
                                       decltype(rep)& vec, SizeType i) -> Var& { return vec[col + i * localDim]; };
            ...
            template <class MatrixProxy, typename SizeType> class LeastComplexityPivot {
              ...
              private:
                ...
                template <class RhsVectorProxy>
                void copyVector(const RhsVectorProxy& from, VectorWrapper<SizeType>& to) const
                {
                    for (SizeType i = 0; i < dim; ++i)
                        to(i) = from(i);
                }
            ...
            template <class MatrixProxy, typename SizeType> class LUDecomposition {
              public:
                ...
                template <class VectorProxyRhs, class VectorProxySolution>
                void computeSolution(VectorProxyRhs&& rhs, VectorProxySolution&& x)
                {
                    for (SizeType i = 0; i < dim; ++i)
                        x(i) = 0;
            ...
            template <class MatrixProxy, class VectorProxy, typename SizeType>
            void solveViaProxies(MatrixProxy& A, VectorProxy& b, VectorProxy& x, SizeType dim)
            {
            ...
            template <class Matrix, class MatrixAccess, class Vector, class VectorAccess, typename SizeType>
            void solve(Matrix& A, MatrixAccess&& mAccess, Vector& b, Vector& x, VectorAccess&& vAccess, SizeType dim)
            {
            ...
            template <class MatrixProxy, typename SizeType>
            Var detFromPlu(MatrixProxy& A, SizeType dim, unsigned nPivotSwaps)
            {
            ...
            template <class Matrix, class MatrixAccess, typename SizeType>
            Var determinant(Matrix& matrix, MatrixAccess&& access, SizeType dim)
            {
            ...
            template <class Matrix, class MatrixAccess, typename SizeType>
            void invert(Matrix& matrix, MatrixAccess&& access, SizeType dim)
            {
                plu::detail::MatrixAccessProxy<Matrix, SizeType> mProxy(matrix, std::forward<MatrixAccess>(access));
    ...
    template <class Matrix, class Vector, typename SizeType, class MatrixAccess = plu::detail::DefaultAccess<Matrix>,
      class VectorAccess = plu::detail::DefaultAccess<Vector>>
    void solve(Matrix& A, Vector& b, Vector& x, SizeType dim, MatrixAccess&& mAccess = MatrixAccess{},
      VectorAccess&& vAccess = VectorAccess{})
    {
        plu::detail::solve(A, std::forward<MatrixAccess>(mAccess), b, x, std::forward<VectorAccess>(vAccess), dim);
    }

    template <class Matrix, typename SizeType, class MatrixAccess = plu::detail::DefaultAccess<Matrix>>
    Var determinant(Matrix& A, SizeType dim, MatrixAccess&& access = MatrixAccess{})
    {
        return plu::detail::determinant(A, std::forward<MatrixAccess>(access), dim);
    }

    template <class Matrix, typename SizeType, class MatrixAccess = plu::detail::DefaultAccess<Matrix>>
    void invert(Matrix& A, SizeType dim, MatrixAccess&& access = MatrixAccess{})
    {
